...
import { ExecutionEvent, ToolCall } from '@/lib/types/hermes';
...
export default function CommandCenter() {
  ...
  // Extract latest events and tool calls for live display
  const lastMsg = messages[messages.length - 1];
  const activeEvents: ExecutionEvent[] = (lastMsg as any)?.events || [];
  const activeTasks = lastMsg?.tasks || [];
          ...
          {messages.length === 0 && !isThinking && (
            <div className="h-full flex flex-col items-center justify-center opacity-20 text-center space-y-4">
               ...
               <div className="space-y-1">
                  ...
                  <p className="text-[10px] uppercase tracking-[0.4em] text-[#94a3b8]">Computer Use & Computer Control Active</p>
               </div>
            </div>
          )}
                  ...
                  <div className="flex justify-end mb-6">
                     <div className="max-w-[70%] bg-[#4d79a0] px-5 py-4 rounded-2xl rounded-tr-none text-white text-[14px] shadow-xl leading-relaxed">
                        ...
                        <div className="flex items-center justify-end gap-1 mt-1 opacity-60">
                           <span className="text-[9px] font-medium uppercase">{msg.timestamp?.slice(0, 5)}</span>
                     ...
                     {msg.tasks && msg.tasks.length > 0 && (
                        <div className="max-w-[85%] bg-[#162029] border border-white/5 rounded-2xl overflow-hidden shadow-2xl">
                           ...
                           <div className="p-4 space-y-3">
                              {msg.tasks.map((task) => (
                                <div key={task.id} className="bg-[#1c2833] border border-white/5 rounded-xl p-3 flex flex-col gap-3">
                                   <div className="flex items-center justify-between">
                                      <div className="flex items-center gap-3">
                                         {task.status === 'running' ? <Loader2 size={14} className="text-[#83e1ff] animate-spin" /> : 
                                          task.status === 'completed' ? <CheckCircle2 size={14} className="text-[#34c759]" /> :
                                          <div className="w-1.5 h-1.5 rounded-full bg-white/10" />}
                                         <span className={`text-[11px] font-bold ${task.status === 'running' ? 'text-white' : 'text-[#94a3b8]'}`}>{task.title}</span>
                                      </div>
                                      <span className="text-[9px] font-mono text-white/20 uppercase tracking-tighter">{task.workerName}</span>
                                   </div>
                                   
                                   {/* TOOL CALLS HONEST DISPLAY */}
                                   {task.toolCalls && task.toolCalls.length > 0 && (
                                     <div className="pl-6 border-l border-white/5 space-y-3">
                                        {task.toolCalls.map((tc) => (
                                          <div key={tc.id} className="flex flex-col gap-2 text-[10px] bg-black/20 p-3 rounded-lg border border-white/5">
                                             <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-2">
                                                   <Zap size={10} className={
                                                      tc.status === 'success' ? 'text-[#34c759]' : 
                                                      tc.status === 'not_implemented' ? 'text-[#ffb000]' : 
                                                      tc.status === 'error' ? 'text-[#ffb4ab]' : 'text-[#83e1ff] animate-pulse'
                                                   } />
                                                ...
                                                <span className={`px-2 py-0.5 rounded text-[8px] font-bold uppercase ${
                                                   ...
                                                   tc.status === 'not_implemented' ? 'bg-[#ffb000]/10 text-[#ffb000] border border-[#ffb000]/30' :
                                             ...
                                             {tc.artifactPath && (
                                                <div className="mt-2 rounded-lg border border-white/10 overflow-hidden bg-black/40 relative group">
                                                   <div className="absolute top-2 right-2 bg-black/60 px-2 py-1 rounded text-[8px] uppercase font-bold text-white/50 group-hover:opacity-0 transition-all z-10">Proof of Action</div>
                                                   {/* Display image artifact if it looks like one */}
                                                   {tc.artifactPath.match(/\.(png|jpg|jpeg|webp)$/i) ? (
                                                      // eslint-disable-next-line @next/next/no-img-element
                                                      <img src={tc.artifactPath} alt="Tool Artifact" className="w-full h-auto max-h-40 object-cover object-top opacity-80 group-hover:opacity-100 transition-all cursor-zoom-in" />
                                                   ) : (
                                                      <div className="p-3 text-[#83e1ff] flex items-center gap-2"><FileText size={12}/> {tc.artifactPath}</div>
                                                   )}
                                                </div>
                                             )}
                        ...
                        <div className="flex justify-start">
                           <div className="max-w-[70%] bg-[#2c3948] px-5 py-4 rounded-2xl rounded-tl-none text-white text-[14px] shadow-xl border border-white/10 leading-relaxed">
                              ...
                              <div className="flex items-center justify-end gap-1 mt-1 opacity-40">
                                 <span className="text-[9px] font-medium uppercase font-mono tracking-tighter">{msg.timestamp?.slice(0, 5)}</span>
                              </div>
                           </div>
                        </div>
                     )}
        ...
        <div className="p-4 md:p-8 bg-gradient-to-t from-[#1c2833] via-[#1c2833] to-transparent">
           <div className="max-w-4xl mx-auto space-y-5">
              <div className="flex items-center gap-2 overflow-x-auto no-scrollbar py-1">
                 {[
                   { label: 'Analiser MacOS', icon: Monitor },
                   ...
                   { label: 'Vision Audit', icon: MousePointer2 }
                    ...
                    <button 
                      ...
                      className="px-4 py-2 rounded-xl bg-[#2c3948] border border-white/5 flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-[#94a3b8] hover:text-white hover:bg-[#3d4d5e] transition-all shadow-lg"
                    >
                       <btn.icon size={13} />
                       {btn.label}
                    </button>
              ...
              <div className="flex items-center gap-4">
                 <button className="w-12 h-12 rounded-xl bg-[#2c3948] border border-white/5 text-[#94a3b8] flex items-center justify-center hover:bg-[#3d4d5e] transition-all">
                    <Settings size={22} strokeWidth={1.5} />
                 </button>
                 
                 <form onSubmit={handleSubmit} className="flex-1 bg-[#2c3948] rounded-[18px] flex items-center px-5 py-3 shadow-2xl border border-white/5 focus-within:border-[#4d79a0]/50 transition-all">
                    <input 
                       type="text"
                       value={input}
                       ...
                       className="flex-1 bg-transparent border-none text-[15px] text-white placeholder-[#94a3b8] focus:outline-none"
                    />
                 </form>
                 ...
                 <button 
                    ...
                    disabled={isThinking}
                    className="w-12 h-12 rounded-xl bg-[#4d79a0] text-white flex items-center justify-center shadow-2xl hover:bg-[#5a8bb6] transition-all transform hover:scale-105 active:scale-95 disabled:opacity-50"
                 >
                    {isThinking ? <RefreshCcw size={22} className="animate-spin" /> : <Play size={22} fill="currentColor" />}
                 </button>
              </div>
           </div>
        </div>
      ...
      <div className="hidden xl:flex w-[380px] h-full flex-col bg-[#162029] border-l border-white/5 animate-in slide-in-from-right duration-500">
         <div className="p-8 border-b border-white/5">
            ...
            <div className="p-5 rounded-2xl bg-white/5 border border-white/5 space-y-4">
               <div className="flex justify-between items-center text-[10px]">
                  ...
                  <span className="text-[#83e1ff] font-mono font-bold uppercase">{status?.brain?.model || 'llama3.1:8b'}</span>
               </div>
         ...
         <div className="p-8 space-y-8 flex-1 overflow-y-auto custom-scrollbar">
               ...
               <div className="space-y-4">
                  {activeEvents.length > 0 ? (
                    activeEvents.map((evt) => (
                      <div key={evt.id} className="relative pl-6 border-l border-white/5 animate-in fade-in slide-in-from-left-2 duration-300">
                         <div className={`absolute left-[-5px] top-1.5 w-2 h-2 rounded-full ${
                            evt.level === 'error' ? 'bg-[#ffb4ab]' : 
                            evt.level === 'tool_call' ? 'bg-[#ffb000]' : 
                            evt.level === 'result' ? 'bg-[#34c759]' : 'bg-[#4d79a0]'
                         } shadow-lg`} />
                         <div className="space-y-1">
                            <div className="flex justify-between text-[9px]">
                               <span className="text-white/80 font-bold uppercase">{evt.source}</span>
                               <span className="text-[#94a3b8] font-mono">{evt.timestamp.slice(14, 19)}</span>
                            </div>
            ...
            <div className="space-y-4">
               ...
               <div className="space-y-4">
                  {[
                    { label: 'CPU Cluster', value: 34, icon: Cpu },
                    ...
                    <div key={i} className="flex items-center gap-4 bg-white/[0.02] p-3 rounded-xl border border-white/5">
                       ...
                       <div className="flex-1 space-y-1.5">
                          ...
                          <div className="h-0.5 bg-white/5 rounded-full overflow-hidden">
                             <div className="h-full bg-[#4d79a0] transition-all duration-1000" style={{ width: `${stat.value}%` }} />
                          </div>
                       </div>
                    </div>
                  ))}
         ...
         <div className="p-8 border-t border-white/5 bg-[#1c2833]">
            <button className="w-full py-4 bg-[#2c3948] rounded-xl text-[10px] font-bold uppercase tracking-[0.2em] text-[#94a3b8] hover:text-white hover:bg-[#3d4d5e] transition-all border border-white/5 flex items-center justify-center gap-2">
